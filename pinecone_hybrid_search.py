...
class PineconeHybridSearchRetriever(BaseRetriever, BaseModel):
    ...
    def add_texts(self, texts: List[str], ids: Optional[List[str]] = None, namespace: Optional[str] = None) -> None:
        create_index(texts, self.index, self.embeddings, self.sparse_encoder, ids=ids, namespace=namespace)
    ...
    def get_relevant_documents(self, query: str) -> List[Document]:
        ...
        for res in result["matches"]:
            document = Document(page_content=res["metadata"]["context"])
            if "source" in res["metadata"]:
                document.metadata["source"] = res["metadata"]["source"]
